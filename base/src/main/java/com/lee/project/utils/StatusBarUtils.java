...
public final class StatusBarUtils {
     ...
     * 设置状态栏是否显示
     *
     * @param isVisible true为显示，false 为隐藏
     */
     ...
     * 设置状态栏的风格，即字体颜色
     *
     * @param isLightMode true为黑色，false为白色
     */
     ...
     * @param color    The status bar's color.
     * @param alpha    The status bar's alpha which isn't the same as alpha in the color.
     * @param isDecor  True to addItem fake status bar in DecorView,
     *                 false to addItem fake status bar in ContentView.
     */
    public static void setStatusBarColor(@NonNull final Activity activity,
                                         @ColorInt final int color,
                                         @IntRange(from = 0, to = 255) final int alpha,
    ...
    public static void setStatusBarAlpha2(@NonNull final Activity activity,
                                          @IntRange(from = 0, to = 255) final int alpha) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
     ...
     * <p>DrawLayout must addItem {@code android:fitsSystemWindows="true"}</p>
     *
     * @param activity      The activity.
    ...
    private static void transparentStatusBar2(final Activity activity) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
    ...
    public static int getActionBarHeight() {
        ...
        if (App.context.getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
            return TypedValue.complexToDimensionPixelSize(
                    tv.data, App.context.getResources().getDisplayMetrics()
            );
        }
        return 0;
    }
    ...
    private static void invokePanels(final String methodName) {
        try {
            ...
            Object service = App.context.getSystemService("statusbar");
    ...
    public static boolean isSupportNavBar() {
        ...
        boolean menu = ViewConfiguration.get(App.context).hasPermanentMenuKey();
